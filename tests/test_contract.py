...
# A repo whose COMMITTED baseline scopes pytest to a subdirectory (``pkg/tests``), with a
# deliberately collection-broken module at the REPO ROOT. A bare-root ``python -m pytest`` (the
# current hardcoded ``_BASELINE``) collects the broken root file and COLLECTION_ERRORs (exit 2),
# while the committed command ``python -m pytest pkg/tests`` scopes past it and is GREEN. This root-
...
_SUBDIR_CONFIG = 'baseline = ["-m", "pytest", "pkg/tests"]\nframework = "pytest"\n'
_SUBDIR_BASE_FILES = {
...
def _subdir_scenario(root: Path, name: str, *, candidate_files: dict[str, str]) -> SimpleNamespace:
    ...
    SUBDIRECTORY and a collection-broken module sits at the REPO ROOT — so a bare-root pytest
    COLLECTION_ERRORs while the committed ``python -m pytest pkg/tests`` command runs green."""
    repo = root / name
    repo.mkdir(parents=True)
    subprocess.run(["git", "init", "-q", "-b", "main", str(repo)], check=True)
...
def test_prove_red_uses_committed_baseline_command_for_subdir_layout(tmp_path):
    ...
    baseline to ``python -m pytest pkg/tests``. A stray module at the repo ROOT fails to import, so a
    bare-root ``python -m pytest`` blows up in collection while ``python -m pytest pkg/tests`` runs
    clean and green. When the candidate adds a genuinely-red test under ``pkg/tests``, prove_red must
    reach the CANDIDATE red proof, not stall on a false "the base is broken".

    technical (contract): committed baseline ``["-m", "pytest", "pkg/tests"]``; base suite
    ...
    scoping to ``pkg/tests``: the base runs GREEN and prove_red reaches ``behavioral_red`` ->
    accepted True.
    """
