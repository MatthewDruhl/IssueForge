# IssueForge v1 — PRD #1 decomposition (draft v3, post-amendment)

**Repo:** MatthewDruhl/IssueForge · **Source PRD:** #1 · **Date:** 2026-07-14
**Supersedes:** `docs/planning/decomposition-draft-v2-SUPERSEDED.md` (rejected twice by the review gate; 0 issues created).

PRD #1 and `docs/prd-v1.md` are identical and current, **amended with D1–D4**.

**59 acceptance criteria / 11 user stories. 25 v1 child issues + 2 `deferred-v2`.**

**Round 2.** The independent gate returned **REVISE** on round 1 with 8 blocking findings. All 8 are fixed; the
corrections are marked inline and summarised in §9.

The prior draft counted **51** criteria. The amendment added **8**. A decomposition reusing the old
matrix silently drops all eight. They are listed below.
...
| **US-3.5** | 45 | Shaping emits a **buildability contract** before any acceptance test is authored. |
| **US-3.6** | 46 | **A human approves that contract — including the file scope — before contract authoring.** The enforced scope is that approved scope, **never derived from the diff** (a diff-derived scope approves itself). |
| **US-3.7** | 47 | **Expanding the approved scope requires new human authorization**; an out-of-scope write pauses the run. |
| **US-5.6** | 69 | The frozen dependency set is **discovered by the adapter, not declared by configuration**, transitively. A user path list may **add** to the boundary, never **shrink** it. |
...
approved file scope), US-6.2 (one budget → **two**), US-6.4 (readiness adds approved file scope + the override
carve-out), US-7.2 (PR reports overrides), US-9.4 (roles not vendors; no provider name hardcoded).
...
**B. The source-audit lint cannot establish completeness** (US-11.1–11.4). It checked against
`architecture.md`'s *"**Initial** source map"* and the ledger's *"including **at minimum**"* list — **both
expressly non-exhaustive**. → **S2** builds a **versioned authoritative inventory discovered by scanning the
MARVIN checkout**, whose fourth failure mode is **discovered-but-uninventoried**. This draft is itself the
proof the defect is real: the verification pass found five canonical MARVIN artifacts the prior draft's audit
had declared not to exist (§3).
...
| G12 the five adapter function names are **nowhere in the PRD**, not even in prose | **S1** — they name the capability list at `prd-v1.md:157`. Recorded as an **implementation-level naming decision**, not a PRD quotation. |
...
|---|---|---|---|---|
| S1 | Process seam, tri-state results, `.issueforge.toml`, verification-adapter interface + `probe` | 0 | US-4.1, G1, G2, G8, G12 | — |
...
| S3 | Register a repository; resolve or refuse its adapter | 0 | US-1.1–1.5 | S1 |
| S4 | Run store + enqueue + stub stage (one locked write path, redacting) | 0 | US-2.1, US-10.1, US-10.3, G5, G6 | S1, S3 |
...
| S9 | **Buildability contract** + human approval of the file scope | 2 | US-3.4, US-3.5, US-3.6, US-6.7, G11 | S5, S7, S8 |
| S10 | Author acceptance tests + deterministic red proof | 3 | US-5.1, US-5.2 | S6, S7, S9, S20 |
| S11 | Independent review of the red contract: semantic validity + recorded override | 3 | US-5.3, US-5.4 | S10 |
...
| S15 | Readiness gate: file scope, code review, human override | 4 | US-3.7, US-6.4, US-6.5, G15 | S14, S8 |
...
| US-1.2 | S3 | US-5.2 | S10 | US-8.2 | **S18** |
...
S25 (IO write seam + paths + boundary AST lint) ─── gates every issue that writes to disk

S1 (process + config + adapter interface + probe)
├── S3 (registry; adapter resolve-or-refuse)
...
S5 + S7 + S8 ── S9 (BUILDABILITY CONTRACT — human approves the file scope)
                 │
...
- **`VerificationAdapter` Protocol — five functions, mandatory, keyed on (framework, reporter):**
  `probe(toolchain)` → capabilities + **pinned reporter version**; `canonical_collect(invocation)` → canonical
  IDs + selection metadata; `classify(native events)` → **phase-aware** outcomes; `discover_contract_dependencies(collection)`
  → the protected closure; `validate_invocation(command/config)` → a frozen, safe execution plan.
  **This issue ships the Protocol, the `Outcome` enum, and pytest's `probe` only.** The other four land in the
  slices that first need them (S6, S12, S13) — a "build the whole adapter" issue would be a horizontal layer.
...
- **The core owns, and no adapter duplicates:** subprocess isolation, a **fresh artifact directory outside the
  ...
  tests" distinction), report parsing, count reconciliation, exit/timeout/signal capture.
...
`canonical_collect` / `classify` (S6), `discover_contract_dependencies` (S12), `validate_invocation` (S13).
...
**Problem (review 02 #5, still live).** US-11.1–11.4 demand a design record inventorying **the corresponding**
MARVIN skills, scripts, tests, and failure-driven updates, with **every** behavior classified **with a
reason**. Draft v2's lint checked against `architecture.md`'s *"**Initial** source map"* and the ledger's
*"including **at minimum**"* list — **both expressly non-exhaustive**. A record can therefore look complete
while omitting an unlisted canonical artifact. **This is not hypothetical: the verification pass for this very
draft found five canonical MARVIN artifacts the prior audit had declared not to exist** (`prune_plan_files.py`,
`schedule_waves.py`, `issues_to_findings.py`, `close_run_for_pr`'s guarded transition, and
`prd-to-issues/SKILL.md`). A curated list cannot establish completeness. Only discovery can.

**User-visible outcome.** `issueforge audit check <stage>` validates a stage's provenance record and exits
non-zero naming the specific gap — including **a canonical MARVIN artifact that exists on disk and is absent
from the inventory**.

**PRD criteria covered.** **US-11.1, US-11.2, US-11.3, US-11.4** (owner — enforced, not aspirational).

**Observable acceptance criteria**
- A **versioned authoritative inventory** of canonical MARVIN source artifacts, **produced by scanning a
  MARVIN checkout** (path supplied at audit time), not read from a hand-curated map. It is a **build-time
  artifact, checked in**; IssueForge has **no runtime dependency** on the checkout (US-11.5).
- A machine-checkable record per stage: `docs/provenance/stages/<stage>.md` with source identifiers
  (path + symbol + line), a **behavior-level classification with a reason** — one of *deterministic engine
  policy* / *AI judgment* / *human approval* / *discard-with-reason* — an **extraction decision**
  (extract / refactor / **rewrite-with-reason**), and **test provenance** (which MARVIN test ports with the code).
- **The inventory unit is a BEHAVIOR, not a file.** US-11.1 requires inventorying the corresponding *"skills,
  scripts, **tests**, and relevant **failure-driven updates**"*, and US-11.2 requires **every** inventoried
  behavior classified **with a reason**. **An artifact-level scan is not enough:** a record can name
  `merged_runner.py`, classify one safeguard in it, and pass — while six other failed-read inversions in the
  same file go uninventoried. **Discovery therefore enumerates, per artifact: its public symbols, its
  associated test files, and the failure-driven updates that touch it** (from commit/issue references in the
  file and its tests), and the lint requires a **disposition for each**.
- **Five failure modes, all tested:** (1) an unclassified inventory entry; (2) an extract/refactor decision
  with **no source-test disposition**; (3) a reused safeguard with **no mapped ported test** (US-11.4);
  (4) **a discovered canonical artifact absent from the inventory** — the mode draft v2 could not express, and
  the one that catches a false "nothing to port"; (5) **a discovered BEHAVIOR (symbol, associated test, or
  failure-driven update) inside an inventoried artifact with no disposition** — the mode that catches a
  half-audited file.
- **Authoritative discovery roots are declared and versioned** (`scripts/`, `skills/`, `context/`, `tests/`,
  and the provenance ledger), so "what counts as canonical" is a checked-in decision, not a reviewer's guess.
- **A stage audit is HUMAN-APPROVED before that stage is implementation-ready.** The lint proves the record is
  *complete*; it cannot prove a classification is *correct*. **"Discard — MARVIN-specific" is exactly the
  judgment a lint cannot make**, and it is the one that silently drops a safeguard.
- **US-11.3:** a rewrite with **no documented reason why extraction was unsuitable** fails the lint.
- A stage may legitimately record **"new engine policy — no MARVIN prior art"** (e.g. the parent-epic update),
  but it must be an **explicit classified entry**, not an omission.
- Reports **every** violation; no fail-fast. Runs in CI. **No implementation issue is ready while its audit is
  incomplete.**

**Expected footprint.** `src/issueforge/audit.py`, `docs/provenance/stages/`, `docs/provenance/inventory.json`,
`tests/test_audit.py`, `cli.py`, CI workflow.

**Dependencies.** Blocked by: none. **Gates every implementation issue; blocks none.**

**Deterministic / AI / Human.** Deterministic lint (completeness). No AI. **Human: authoring each
classification, and APPROVING the stage audit.**

**Human approval points.** **The stage audit itself, before that stage is implementation-ready.** The lint
proves completeness; only a human can judge whether a "discard" is correct.
...
- *Replace:* the artifact schema and the discovery-based inventory — net-new.
...
**Dependencies.** Blocked by **S1**. Unblocks **S4**, **S6**.
...
**Dependencies.** Blocked by **S1**, **S3**. Unblocks **S5, S6, S7, S12, S22**. **Must land before any other stage.**
...
- Additional issues enter a **persistent FIFO queue**; **reorder** and **cancel** have named CLI verbs (G9),
  available **only before a run starts** (US-2.2).
...
Delivers the adapter's **`canonical_collect`** and **`classify`** (from S1's Protocol).
...
(+`canonical_collect`, +`classify`), `engine.py`; `tests/test_workspace.py`, `test_verify.py`, `test_adapters.py`.
...
- The implementation is a heuristic feeding a judgment call: a module-level tuple of boundary markers, one
  `classify(diff)` used as a **hint plus a required contract field**, and one paragraph appended to the
  reviewer prompt. **Not a rule engine. The boundary list is NOT user-configurable in v1.**
...
## S9 — Buildability contract + human approval of the file scope
**Labels:** `v1` `phase:2` `route:spec-up`

**Problem (D3).** At readiness the engine must evaluate "approved file scope" (US-6.4). With no approved set
there are only three possible behaviors and **all three are unacceptable**: fail closed (every run pauses — the
buildable path does not actually work), skip the check (ship a readiness gate that **cannot enforce US-6.4**),
or **approve after seeing the diff — and every diff would approve itself.** The PRD now says so outright
(`prd-v1.md:46`). **A "pass-through shaper" is not a real thing.** The same failure hits observability
concretely: with no pre-implementation contract naming sensitive fields, US-6.9 has **no list to enforce**.
...
human approves it — **including the file scope** — or the run pauses.
...
- **The buildability contract (US-3.5) emits, before any test is authored:** a readiness classification
  ...
  **unresolved-design-decision list**; the **proposed expected footprint** as **allowed files and path
  patterns with justification** plus an explicit `footprint_known` verdict; the **observability verdict**
...
- **US-3.6 — the human approves the buildability contract, including the file scope, BEFORE contract authoring
  begins.** The scope enforced at PR readiness is **exactly this approved scope**; a test asserts it is
...
**AI:** the readiness assessment, the duplicate judgment, the proposed footprint, the observability justification.
**Human: APPROVES the buildability contract, including the file scope. This is the gate.**

**Human approval points.** **One, and it is load-bearing: approval of the buildability contract (file scope
included) before contract authoring.** Also: resolving any pause.
...
**PRD criteria covered.** **US-5.1, US-5.2** (owner).
...
- **PHASE-BASED, never exception-type-whitelisted.** A first red test for a not-yet-existing function
  **legitimately** raises ImportError/AttributeError **in the call phase**. A rule of *"must be
  AssertionError"* is **WRONG and breaks real TDD** (the genuine-TDD trap). **Test BOTH:** a missing-symbol red
  (**VALID**) and a missing-fixture red (**INVALID**), and require the checker to separate them.
- **US-5.2 — the preexisting baseline stays green.** *(This is review-02 defect C: the naive mechanism is
  ...
  **The mechanism — a set operation anchored on the BASE snapshot, never a subtraction from the candidate:**
  ...
  - **The baseline run executes EVERY id in `BASE_IDS`, at the candidate**, in a hermetic environment. It is
...
- The failure representation (exception type, message, unit id, assertion line) is captured **VERBATIM** into
  ...
  per-unit verdict. **IssueForge has no PENDING-on-main self-reporting artifact, so the manifest's red evidence
  is the ONLY record — it must be a runner capability, not a string someone wrote down once.**
...
**PRD criteria covered.** **US-5.3, US-5.4** (owner).
...
- **US-5.6 — `discover_contract_dependencies` returns the protected closure. The ROOTS ARE SPECIFIED
  ...
  - **IN, and easy to miss: EXTERNAL package identities.** US-5.6 says *"plugins, and their **transitive**
    ...
    every hash intact. **`probe` supplies pinned versions** (S1) and the manifest freezes the **immutable
    identity + version** of every external package in the closure.
  - **OUT, and this is load-bearing: the SYSTEM UNDER TEST.** A test's import of the production module it
    ...
    a test's outcome*, **not the code the test is about**. The discriminator is the **approved file scope**
    (US-3.6): a path inside the approved scope is the system under test and is **expected to change**; a path
    outside it that feeds test collection or outcome is **protected**. **A path in BOTH is a contradiction and
    fails the freeze, naming it** — the human resolves it by amending the scope or the contract.
  - **Two fixtures prove both directions, and neither is optional:** (1) `conftest.py → helpers.py`, where only
    `helpers.py` is edited — **caught**; (2) an imported production module inside the approved scope, edited by
    the implementer — **still editable, build proceeds.**
...
- **the APPROVED FILE SCOPE carried forward from the buildability contract (US-3.6/US-5.5)** — the amendment's
  addition, and the reason the freeze can be checked against a scope at readiness at all.
...
- **US-6.1: implementation cannot proceed to PR readiness** when contract files, collection, configuration, or
  command changed **without new human authorization**.
...
- **The AUTHORITATIVE test run is the ENGINE's, not the agent's.** The implementer runs tests as feedback; the
  authoritative run executes the **verbatim proof command** plus the full suite **after the session ends**.
...
## S15 — Readiness gate: file scope, independent code review, human override
**Labels:** `v1` `phase:4` `route:spec-up`
...
- **US-6.4 — PR readiness requires ALL of:** green acceptance tests; **green full baseline (a SEPARATE gate —
  this slice may not regress any previously-passing test)**; configured quality gates; **approved file scope**;
  and an independent code review with **no blocking findings except findings explicitly overridden under
  US-6.5**. Plus the **observability reconciliation** below.
- **The observability reconciliation is DETERMINISTIC and NON-OVERRIDABLE.** The gate runs S8's
  ...
  the override a legal path to ship an unlogged boundary crossing. **Also deterministically verified here:** the
  required success/failure events are emitted, the target project's logging conventions are followed (US-6.9),
  and **no contract-listed sensitive field appears in any emitted log.**
- **The approved file scope is the one a human approved in S9. A test asserts it is NEVER derived from the
  diff** — *a scope derived from the diff approves itself.*
- **US-3.7 — an implementation that writes OUTSIDE the approved scope PAUSES the run.** **Expanding the scope
  requires NEW human authorization**, and **the prior approval is preserved in the audit trail**. Defined,
  tested behavior for `add` / `modify` / `delete` / `rename` (S9's path-operation vocabulary).
...
- **US-6.5 — the override:**
  ...
  - **it can NEVER waive** contract integrity, acceptance tests, the full baseline, configured quality gates,
    approved file scope, or deterministic observability/sensitive-data requirements (each asserted separately);
...
**Problem.** A vague issue produces a vague contract. **S9 already classified it and got its file scope
approved**; this slice proposes the **in-place revision** and performs the **first GitHub mutation in the
system** — behind a human gate.
...
**Observable acceptance criteria**
- **ONE IO seam. ALL filesystem mutation routes through it.** The seam makes a **runtime assertion**: every
  target path resolves under **IssueForge's own state root** or **the registered target repo's WORKTREE** —
  and **never** the registered **normal checkout**, and never anywhere else. A violation **raises**.
...
**Dependencies.** Blocked by: none. **Gates every implementation issue that writes to disk (S4 onward).**
