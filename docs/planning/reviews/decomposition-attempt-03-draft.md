# IssueForge v1 — PRD #1 decomposition (draft v4, post-D5/D6 + round-1 gate repairs)

**Repo:** MatthewDruhl/IssueForge · **Source PRD:** #1 · **Date:** 2026-07-15
**Supersedes:** `docs/planning/decomposition-draft-v3.md` as the starting point for the next run.
**Status: still BLOCKED.** This draft was produced by attempt 3 (`/prd-to-issues`). It applies D5/D6 and the
three mechanical fixes, and it fixed the four blocking findings the first review round raised — but **the second
review round returned REVISE with three remaining blocking findings, so attempt 3 created 0 GitHub issues.** The
three open findings are recorded, verified against the PRD, and given a resolution direction in
`docs/planning/issueforge-v1-decomposition-report-v3.md` (§1). **The next run should reuse THIS draft (v4) as
its starting point, resolve those three findings first, then re-run the two-round gate.** Everything the two
review rounds confirmed as resolved (D5, D6, the three mechanical fixes, and round-1 B1/B2/B4) is preserved
...
S25 (IO write seam + paths + boundary AST lint) ─── gates every issue that writes to disk (S3 onward)
...
- **`VerificationAdapter` Protocol — five functions, mandatory, keyed on (framework, reporter):**
  ...
  IDs + selection metadata; `classify(native events)` → **phase-aware** outcomes;
  `discover_contract_dependencies(collection)` → the protected closure — **in-repo file paths AND the immutable
  identity + pinned version of every external plugin/package in the import closure** (US-5.6), resolved against
  the installed distributions in the authoritative environment; `validate_invocation(command/config)` → a
  frozen, safe execution plan. *(Round-1 fix: the external identity/version inventory is
  `discover_contract_dependencies`'s output — `probe` supplies only the reporter version, which cannot enumerate
  the closure. S12 freezes this operation's output; S13 re-runs the SAME operation and compares.)*
...
- **US-6.7 requires the justification to be REVIEWER-CONFIRMED, not merely author-asserted** (`prd-v1.md:82`).
  ...
  A reviewer failure here is recorded and, like US-5.4, may be explicitly overridden by a fresh same-provider
  session or a human — never silently skipped.
...
- **EVERY frozen EXTERNAL identity/version is RE-RESOLVED and COMPARED in the AUTHORITATIVE VERIFICATION
  ...
  `discover_contract_dependencies` in the authoritative verification environment and compares each external
...
- **US-6.4 — PR readiness requires ALL of:** green acceptance tests; **green full baseline (a SEPARATE gate —
  this slice may not regress any previously-passing test)**; configured quality gates; the **two D5 scope
...
- **A CI-ORDER ASSERTION: no module that writes to disk can land before the seam and lint are active** (*v2-report
  mechanical fix 3*). Because S3 (registry) and S4 (run store) both persist to disk, **S25 is a hard blocker of
  both** — the write seam and boundary lint must exist in the tree before, and be enforced by CI on, the very
  first persisting slice. A test asserts the CI workflow runs `lint boundary` as a required check on every PR
  touching `src/issueforge/`, so the control cannot be added *after* a persisting module already shipped.
...
**Dependencies.** Blocked by: none. **Gates every implementation issue that writes to disk — S3 (registry) and
S4 (run store) onward** (*v2-report mechanical fix 3: S25 hard-blocks S3 and S4, not "S4 onward"*).
...
*End of final draft (post-D5/D6, round-1 gate repaired).*
